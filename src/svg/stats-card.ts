...
import { THEMES, formatNumber, escapeXml, type Theme } from "./utils.ts";

export function generateStatsCard(stats: GitHubStats, theme: Theme = "dark"): string {
  const c = THEMES[theme];
  const { user, stats: s } = stats;
  const title = escapeXml(user.name ?? user.login);

  const items = [
    { label: "Total Stars", value: formatNumber(s.totalStars), icon: "⭐" },
    { label: "Total Forks", value: formatNumber(s.totalForks), icon: "🍴" },
    { label: "Total Commits", value: formatNumber(s.totalCommits), icon: "📝" },
    { label: "Contributed To", value: formatNumber(s.contributedRepos), icon: "🔀" },
    { label: "Repositories", value: formatNumber(user.public_repos), icon: "📦" },
  ...
  <style>
    .header { animation: fadeIn 0.8s ease-in-out forwards; }
